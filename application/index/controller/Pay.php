...
class Pay extends Common
{
    public function checkout()
    {
        ...
        // 渲染订单页数据
        $cart_info = model('Cart')->getList();
        $this->assign('data', $cart_info);
        // halt($cart_info);
        $total = model('Cart')->getTotal($cart_info);
        $this->assign('total', $total);
        // 检查商品总价格
        if ($total == 0)
            $this->error('您还未购入任何商品哦', 'index/index');
        return $this->fetch();
    }
    public function checkLogin()
    {
        ...
        if (!$user_info) {
            // 传参数表示当前登入之后跳转页面
            $this->error('请先登入', 'User/Login?cart=1');
        }
    }
    public function pay()
    {
